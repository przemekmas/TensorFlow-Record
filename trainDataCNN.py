...
def flower_input(if_random = True, if_training = True):
    if(if_training):
        filenames = [os.path.join(CURRENT_DIR, "train-0000%d-of-00002" % i) for i in xrange(0, 1)]
    else:
        filenames = [os.path.join(CURRENT_DIR, "train-0000%d-of-00002" % i) for i in xrange(0, 1)]
...
#    loss = tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(labels=label_batch_one_hot, logits=logits_out))
    ...
    with tf.Session() as sess:
        ...
        saver.restore(sess, CURRENT_DIR+"/datamodel.ckpt")
        ...
        for i in range(TRAINING_SET_SIZE * 100):
            ...
            print(loss_out)
            if(i%50 == 0):
                saver.save(sess, CURRENT_DIR+"/datamodel.ckpt")
